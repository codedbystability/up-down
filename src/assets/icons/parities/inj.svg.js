import * as React from "react"

const InjUSDT = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 56 56">
        <path fill="url(#azizqwxii)" d="M0 0h56v56H0z"/>
        <path
            d="M15.88 17.53c.2-.25.4-.49.62-.73l.04-.03.07-.06.02-.02a7.2 7.2 0 0 1 2.48-1.5 6.45 6.45 0 0 1 6.67 1.54c2.73 2.55 2.49 6.67.3 9.4-2.74 4.08-7.47 9.77-.92 14.87 1.17.91 2.05 1.67 5.75 2.74-2.42.45-4.67.3-7.17-.33-1.77-1-4.55-3.14-5.5-6.03-1.43-4.38 2.52-10.92 4.43-13.44 2.62-3.5-1.62-7.27-4.74-3.05-1.63 2.2-4.49 8.41-3.5 13.03.59 2.61 1.36 4.52 4.42 7.14-.57-.34-1.12-.72-1.65-1.14a16.17 16.17 0 0 1-1.32-22.39Z"
            fill="url(#bzizqwxii)"/>
        <path
            d="M40.12 38.47c-.2.25-.4.49-.62.73l-.04.03-.07.06-.02.02a7.2 7.2 0 0 1-2.48 1.5c-2.23.8-4.72.3-6.67-1.54-2.73-2.55-2.48-6.67-.3-9.4 2.74-4.08 7.47-9.77.92-14.87-1.17-.91-2.05-1.67-5.75-2.74a16.1 16.1 0 0 1 7.17.33c1.77 1 4.55 3.14 5.5 6.03 1.43 4.38-2.52 10.92-4.43 13.44-2.62 3.5 1.62 7.27 4.74 3.05 1.63-2.2 4.49-8.41 3.5-13.03-.59-2.61-1.36-4.52-4.42-7.14.57.34 1.12.72 1.65 1.14a16.17 16.17 0 0 1 1.32 22.39Z"
            fill="url(#czizqwxii)"/>
        <defs>
            <linearGradient id="azizqwxii" x1="10.42" y1="9.71" x2="68.15" y2="76.02" gradientUnits="userSpaceOnUse">
                <stop stopColor="#1A1E21"/>
                <stop offset="1" stopColor="#06060A"/>
            </linearGradient>
            <linearGradient id="bzizqwxii" x1="12" y1="28" x2="44" y2="28" gradientUnits="userSpaceOnUse">
                <stop stopColor="#0082FA"/>
                <stop offset="1" stopColor="#00F2FE"/>
            </linearGradient>
            <linearGradient id="czizqwxii" x1="12" y1="28" x2="44" y2="28" gradientUnits="userSpaceOnUse">
                <stop stopColor="#0082FA"/>
                <stop offset="1" stopColor="#00F2FE"/>
            </linearGradient>
        </defs>
    </svg>
)

export default InjUSDT
